...
visible_trees <- function(trees, tree) {
  ...
  if_else(visible < length(trees), visible + 1, visible + 0)
}
...
scenic_score <- function(i, j, forest){
  ...
  trees_in_nswe_dirs(i, j, forest) |>
    map_dbl(~visible_trees(., forest[i, j])) |>
    prod()
}

forest <- read_lines('input/input-08.txt') |>
  map(strsplit, '') |>
  unlist() |>
  as.numeric() |>
  {\(x) matrix(x, nrow = sqrt(length(x)), ncol = sqrt(length(x)), byrow = T)}()

part1 <- crossing(i = seq(ncol(forest)), j = i) |>
